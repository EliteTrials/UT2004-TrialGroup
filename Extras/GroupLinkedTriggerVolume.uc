...
class GroupLinkedTriggerVolume extends TrialGroup.GroupTriggerVolume;

var() const byte RequiredMembers;

var GroupLinkedVolumesManager LinkManager;
...
function bool HasAllMembers( int groupIndex, optional out int missingMembers, optional out array<Controller> foundMembers )
{
	...
	missingMembers = (RequiredMembers - Min( foundMembers.Length, RequiredMembers ));
	// Return True if we found more members than need and that the group is indeed at it capacity.
	return (foundMembers.Length >= RequiredMembers && Manager.Groups[groupIndex].Members.Length == Manager.MaxGroupSize);
}

defaultproperties
{
	Info="Atleast an amount of (RequiredMembers) members of a group(group must also be full) must enter this volume in order to cause the volume to trigger the specified event."
}
